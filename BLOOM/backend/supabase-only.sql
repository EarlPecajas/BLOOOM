...
-- user_profiles: readable/updatable by the owning user; insertable by service role
-- (The table and policies are defined in centralize-users-and-drafts.sql;
--  this grant ensures the permissions survive a supabase-only.sql re-run.)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema = 'public' AND table_name = 'user_profiles') THEN
    EXECUTE 'GRANT SELECT, UPDATE ON user_profiles TO authenticated';
    EXECUTE 'GRANT INSERT ON user_profiles TO service_role';
  END IF;
END $$;
