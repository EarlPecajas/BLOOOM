-- Adds sighting_media.uploaded_at (see add-uploaded-at-to-sighting-media.sql,
-- must be run first) to each contributed-photo JSONB object in
-- public_approved_sightings, so the catalog can show a timestamp alongside
-- the existing "contributor" name on each photo.
--
-- This re-creates the view defined in
-- gate-public-sightings-by-catalog-published.sql verbatim, changing only the
-- three jsonb_build_object(...) calls for whole_plant/closeup_flower/habitat
-- photos. Keep this file's body in sync with that one if the view changes
-- again later.
--
...
SELECT
  ...
  -- (uploaded_at added so the catalog can show when each photo was contributed)
  (SELECT coalesce(jsonb_agg(
     jsonb_build_object('src', pic.file_path, 'contributor', smed.photographer_name, 'uploaded_at', smed.uploaded_at)
