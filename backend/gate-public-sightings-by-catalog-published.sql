-- Fixes two DENR "Publish to Catalog" bugs that both trace back to the same
-- root cause: publish/unpublish state lived entirely on the shared
-- `orchids` row (one per species), while `species_sightings.catalog_published`
-- was written per-sighting but never actually read anywhere.
--
-- Bug 1: unpublishing one sighting of a multi-sighting species (e.g. Vanda
-- sanderiana with 4 sightings) unpublished all 4 — because "unpublish"
-- called unpublish_orchid(sci_name), which deletes the ENTIRE species row
-- from `orchids`, and the DENR dashboard's publish/unpublish button state
-- was computed purely from species-name membership in that table.
-- Bug 2: hiding a detail field (hidden_fields, also stored on that same
-- shared `orchids` row) got silently wiped any time any sighting of that
-- species was unpublished, since the whole row — hidden_fields included —
-- was deleted.
--
-- Fix: make `species_sightings.catalog_published` the actual per-sighting
-- visibility gate for public_approved_sightings, and stop deleting the
-- shared `orchids` row on a per-sighting unpublish (see the matching
-- denr-dashboard.html changes — doUnpublish no longer calls
-- unpublish_orchid()). A sighting with catalog_published = NULL (never
-- explicitly toggled — true of every sighting published before this fix)
-- is treated as visible, so nothing that's currently public disappears.
--
...
ALTER TABLE species_sightings
  ADD COLUMN IF NOT EXISTS catalog_published BOOLEAN;
